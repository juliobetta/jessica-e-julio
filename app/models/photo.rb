class Photo < ActiveRecord::Base
  ...
  DIRECT_UPLOAD_URL_FORMAT = %r{\Ahttps:\/\/s3\.amazonaws\.com\/jessicaejulio#{!Rails.env.production? ? "\\-#{Rails.env}" : ''}\/(?<path>uploads\/.+\/(?<filename>.+))\z}.freeze
  ...
  def set_upload_attributes
    ...
    self.data_file_name     = direct_upload_url_data[:filename]
    self.data_file_size     = direct_upload_head.content_length
    self.data_content_type  = direct_upload_head.content_type
    self.data_updated_at    = direct_upload_head.last_modified
